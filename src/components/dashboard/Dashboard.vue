<template>
  <section class="bg-bgLight min-h-screen flex">
    ...
    <button
      @click="toggleSidebar"
      class="text-white fixed top-4 left-4 z-50 bg-primaryMedium rounded-full p-2 hover:bg-primaryDark transition"
    >
    ...
    >
      ...
      <ul class="space-y-4">
        <li>
          <router-link
            ...
            :class="{
              'font-semibold text-blue-600': $route.path === '/profile',
            }"
          >
            Profile Settings
          </router-link>
      ...
      <button
        @click="logout"
        class="mt-auto bg-primaryMedium text-white py-2 rounded-lg w-full hover:bg-primaryDark transition"
      >
        Log Out
      </button>
    </aside>
    ...
    <div class="flex-1 p-8 space-y-8">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="card w-40">
          <div
            class="card__content text-center relative p-20 transition-transform duration-1000 text-white font-bold"
          >
            <div
              class="card__front absolute top-0 bottom-0 right-0 left-0 p-8 bg-primaryLight flex items-center justify-center"
            >
            ...
            <div
              class="card__back absolute top-0 bottom-0 right-0 left-0 p-8 bg-primaryLight flex items-center justify-center"
            >
              <h2 class="text-xl font-semibold text-gray-800">
                Detailed Overview
              </h2>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        >
          ...
          <router-link
            :to="widget.link"
            class="bg-primaryMedium text-white px-4 py-2 rounded-lg hover:bg-primaryDark transition"
          >
            Go to {{ widget.title }}
          </router-link>
        </div>
...
const widgets = [
  ...
  },
  {
    ...
    description: "Analyze marketing",
    link: "/analysis",
    icon: "pi pi-chart-line",
  },
  {
    title: "Trend Seeker",
    description: "Find latest trends",
    link: "/search",
    icon: "pi pi-search",
  },
];
  ...
  if (!user) {
    ...
    if (userDoc.exists()) {
      ...
      userProfilePic.value =
        userDoc.data().profilePicture || userProfilePic.value;
    }
  }
});
</script>
